...
TEMPLATE ?= 8.1-supervisord-alpine

IMAGE_TAG ?= $(IMAGE_NAMESPACE):$(TEMPLATE)-latest
DOCKERFILE_DIR ?= ./dist/prod/$(TEMPLATE)
...
MAKE_LOGFILE = /tmp/docker-php-prod.log
MAKE_CMD_COLOR := $(BLUE)
...
help:
	...
	@echo
	@echo '    📦 Package                 docker-php-prod (github.com/wayofdev/docker-php-prod)'
...
ssh: ## Login into built image
	docker run --rm -it -v $(PWD)/:/opt/docker-php-prod $(IMAGE_TAG) sh
.PHONY: ssh

hadolint: ## Run hadolint over dist Dockerfiles
	hadolint -V ./dist/prod/7.4-cli-alpine/Dockerfile
	hadolint -V ./dist/prod/7.4-fpm-alpine/Dockerfile
	hadolint -V ./dist/prod/7.4-supervisord-alpine/Dockerfile
	hadolint -V ./dist/prod/8.0-cli-alpine/Dockerfile
	hadolint -V ./dist/prod/8.0-fpm-alpine/Dockerfile
	hadolint -V ./dist/prod/8.0-supervisord-alpine/Dockerfile
	hadolint -V ./dist/prod/8.1-cli-alpine/Dockerfile
	hadolint -V ./dist/prod/8.1-fpm-alpine/Dockerfile
	hadolint -V ./dist/prod/8.1-supervisord-alpine/Dockerfile
.PHONY: hadolint
